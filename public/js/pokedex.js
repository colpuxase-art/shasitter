(() => {
  ...
  const state = { clients: [], prestations: [], upcoming: [], past: [], compta: null, activePanel: 'home' };

  const money = n => (Math.round(Number(n || 0) * 100) / 100).toFixed(2);
  const todayISO = () => new Date().toISOString().slice(0, 10);

  const dailyMessages = [ /* tes 10 messages */ ];
  ...
  // ACCUEIL - fond sombre + pastilles bien visibles
  function renderHome() {
      ...
      html += `
        <div class="accordion-item">
          <h2 class="accordion-header">
            <button class="accordion-button ${i===0?'':'collapsed'}" data-bs-toggle="collapse" data-bs-target="#d${i}">
              📅 ${day} <span class="badge bg-warning text-dark ms-3">${grouped[day].length}</span>
            </button>
          </h2>
  ...
  // COMPTA - chiffres qui rentrent + sélection client qui marche
  ...
  // Les autres fonctions (renderClients, renderPrestations, renderBookings) restent identiques à ce que tu avais.
