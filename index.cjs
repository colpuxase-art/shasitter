...
/* ================== CALLBACKS ================== */
/* ================== SMART BOOKING (split + supplements + pagination) ==================
   Objectif:
   - Prestations “manquantes”: pagination + filtres (animal + visits_per_day)
   - Suppléments: category='supplement' + quantité
   - Calcul “semaine”: 1er jour soir + jours du milieu matin_soir + dernier jour matin => split en 1..3 bookings
*/

const wSmart = new Map(); // chatId -> { step, data, history }

function sbSet(chatId, st) { wSmart.set(chatId, st); }
function sbGet(chatId) { return wSmart.get(chatId); }
function sbCancel(chatId) { wSmart.delete(chatId); }

function sbPush(st, step) { st.history = st.history || []; st.history.push(step); }
function sbPop(st) { st.history = st.history || []; return st.history.pop(); }

function sbNavRow() {
  return [{ text: "⬅️ Retour", callback_data: "sb_back" }, { text: "❌ Annuler", callback_data: "sb_cancel" }];
}
...
async function sbRenderPickPrestation(chatId, title, st, keyToStore, opts) {
  const all = await dbListPrestations(true);
  const list = filterPrestationsForBooking(all, opts);

  const pageSize = 10;
  st.data._sb_pick = { keyToStore, opts, title };
  st.data._sb_page = st.data._sb_page ?? 0;

  const totalPages = Math.max(1, Math.ceil(list.length / pageSize));
  const page = Math.min(st.data._sb_page, totalPages - 1);
  st.data._sb_page = page;

  const slice = list.slice(page * pageSize, page * pageSize + pageSize);

  const rows = slice.map((p) => [
    { text: `🧾 ${p.name} • ${p.price_chf} CHF`, callback_data: `sb_pickpresta_${p.id}` },
  ]);

  const nav = [];
  if (page > 0) nav.push({ text: "⬅️", callback_data: "sb_prev" });
  nav.push({ text: `Page ${page + 1}/${totalPages}`, callback_data: "noop" });
  if (page < totalPages - 1) nav.push({ text: "➡️", callback_data: "sb_next" });
  rows.push(nav);

  rows.push([{ text: "⬅️ Retour", callback_data: "sb_back" }]);

  sbSet(chatId, st);
  return bot.sendMessage(chatId, title, { parse_mode: "Markdown", ...kb(rows) });
}

async function sbRenderSupplements(chatId, st) {
  const all = await dbListPrestations(true);
  const sups = (all || []).filter((p) => p.active !== false && p.category === "supplement");
  ...
  const rows = sups.slice(0, 20).map((p) => [
    { text: `➕ ${p.name} • ${p.price_chf} CHF`, callback_data: `sb_sup_${p.id}` },
  ]);

  rows.push([{ text: "✅ Terminer suppléments", callback_data: "sb_sup_done" }]);
  rows.push([{ text: "⬅️ Retour", callback_data: "sb_back" }]);
...
async function sbRender(chatId) {
  const st = sbGet(chatId);
  if (!st) return;
  ...
  if (st.step === "pick_client") {
    const clients = await dbListClients();
    const rows = [
      [{ text: "➕ Nouveau client", callback_data: "cl_add" }],
      ...clients.slice(0, 20).map((c) => [{ text: `👤 ${c.name} (#${c.id})`, callback_data: `sb_client_${c.id}` }]),
      [{ text: "⬅️ Retour", callback_data: "back_main" }],
    ];
  ...
  if (st.step === "pick_pet") {
    const pets = await dbListPetsByClient(d.client_id, true);
    const rows = [
      [{ text: "➕ Ajouter un animal", callback_data: "pet_add_for_booking" }],
      ...pets.slice(0, 20).map((p) => [{ text: `🐾 ${p.name} (${animalLabel(p.animal_type)})`, callback_data: `sb_pet_${p.id}` }]),
      [{ text: "⬅️ Retour", callback_data: "sb_back" }],
    ];
  ...
  if (st.step === "slot_single") {
    return bot.sendMessage(chatId, "5/8 — Choisis le créneau (1 jour) :", {
      parse_mode: "Markdown",
      ...kb([
        [{ text: "🌅 Matin (1 visite)", callback_data: "sb_slot_single_matin" }],
        [{ text: "🌙 Soir (1 visite)", callback_data: "sb_slot_single_soir" }],
        [{ text: "🌅🌙 Matin+soir (2 visites)", callback_data: "sb_slot_single_matin_soir" }],
        [{ text: "⬅️ Retour", callback_data: "sb_back" }],
      ]),
    });
  }
  ...
  if (st.step === "slot_start") {
    return bot.sendMessage(chatId, "5/8 — Début : quel créneau le *1er jour* ?", {
      parse_mode: "Markdown",
      ...kb([
        [{ text: "🌅 Matin (1 visite)", callback_data: "sb_slot_start_matin" }],
        [{ text: "🌙 Soir (1 visite)", callback_data: "sb_slot_start_soir" }],
        [{ text: "🌅🌙 Matin+soir (2 visites)", callback_data: "sb_slot_start_matin_soir" }],
        [{ text: "⬅️ Retour", callback_data: "sb_back" }],
      ]),
    });
  }
  ...
  if (st.step === "slot_end") {
    return bot.sendMessage(chatId, "6/8 — Fin : quel créneau le *dernier jour* ?", {
      parse_mode: "Markdown",
      ...kb([
        [{ text: "🌅 Matin (1 visite)", callback_data: "sb_slot_end_matin" }],
        [{ text: "🌙 Soir (1 visite)", callback_data: "sb_slot_end_soir" }],
        [{ text: "🌅🌙 Matin+soir (2 visites)", callback_data: "sb_slot_end_matin_soir" }],
        [{ text: "⬅️ Retour", callback_data: "sb_back" }],
      ]),
    });
  }
  ...
  if (st.step === "supplements") {
    return sbRenderSupplements(chatId, st);
  }

  if (st.step === "share_employee") {
    const emps = await dbListEmployees();
    const rows = [[{ text: "— Aucun employé", callback_data: "sb_emp_none" }]];
    rows.push(...emps.slice(0, 20).map((e) => [{ text: `👩‍💼 ${e.name} (défaut ${e.default_percent}%)`, callback_data: `sb_emp_${e.id}` }]));
    rows.push([{ text: "⬅️ Retour", callback_data: "sb_back" }]);
  ...
  if (st.step === "recap") {
    const supTotal = sbSuppTotal(st);
    const segs = sbBuildSegments(st);
    ...
    const split = (function computeSplit(total, employee_id, employee_percent) {
      if (!employee_id) return { employee_percent: 0, employee_part_chf: 0, company_part_chf: money2(total) };
    ...
    })(total, d.employee_id, d.employee_percent);
    ...
    return bot.sendMessage(
        ...
        `ShaSitter: *${split.company_part_chf} CHF*`,
      {
        parse_mode: "Markdown",
        ...kb([
          [{ text: "✅ Confirmer", callback_data: "sb_confirm" }],
          [{ text: "⬅️ Retour", callback_data: "sb_back" }],
          [{ text: "❌ Annuler", callback_data: "sb_cancel" }],
        ]),
      }
    );
  }
...
bot.on("callback_query", async (q) => {
  ...
  if (q.data === "m_book") {
    ...
    wBooking.delete(chatId); // stop old wizard if any
    sbSet(chatId, { step: "pick_client", data: {}, history: [] });
    return sbRender(chatId);
  }
...
/* ===== SMART BOOKING TEXT INPUT ===== */
const sb = sbGet(chatId);
if (sb) {
  // pending supplement qty
  if (sb.data?._pending_sup) {
    const qty = Number(text);
    if (!Number.isFinite(qty) || qty <= 0) {
      await bot.sendMessage(chatId, "❌ Mets une quantité > 0 (ex: 1, 2, 3)");
      return;
    }
    const sup = sb.data._pending_sup;
    sb.data.supplements = sb.data.supplements || [];
    sb.data.supplements.push({
      id: sup.id,
      name: sup.name,
      qty: Math.floor(qty),
    ...
    delete sb.data._pending_sup;
    sbSet(chatId, sb);
    await sbRenderSupplements(chatId, sb);
    return;
  }

  if (sb.step === "start_date") {
    if (!isValidISODate(text)) {
      await bot.sendMessage(chatId, "❌ Format attendu: YYYY-MM-DD");
      return;
    }
    sbPush(sb, sb.step);
    sb.data.start_date = text;
    sb.step = "end_date";
    sbSet(chatId, sb);
    await sbRender(chatId);
    return;
  }
  ...
  if (sb.step === "end_date") {
    ...
    sbPush(sb, sb.step);
    sb.data.end_date = text;

    const n = daysInclusive(sb.data.start_date, sb.data.end_date);
    if (n <= 0) {
      await bot.sendMessage(chatId, "❌ Date fin avant date début.");
      return;
    }

    sb.step = n === 1 ? "slot_single" : "slot_start";
    sbSet(chatId, sb);
    await sbRender(chatId);
    return;
  }

  if (sb.step === "employee_percent") {
    const p = Number(text);
    if (!Number.isFinite(p) || p < 0 || p > 100) {
      await bot.sendMessage(chatId, "❌ Mets un nombre 0-100");
      return;
    }
    sbPush(sb, sb.step);
    sb.data.employee_percent = Math.floor(p);
    sb.step = "recap";
    sbSet(chatId, sb);
    await sbRender(chatId);
    return;
