...
const ADMIN_IDS = new Set([
  6675436692, // <-- ton ID Telegram (modifie si besoin)
]);
...
/* ================== Telegram initData validation (PRIVATE APP) ==================
   ...
   avec l'initData (Telegram WebApp).
*/
...
function checkTelegramInitData(initData, botToken) {
  ...
  // optional: auth_date freshness (not strict)
...
/* ================== API (Dashboard affiche seulement) ================== */
app.get("/api/prestations", requireAdminWebApp, async (req, res) => {
  try {
    const data = await dbListPrestations();
    res.json(data);
  } catch (e) {
...
app.get("/api/employees", requireAdminWebApp, async (req, res) => {
  try {
    const data = await dbListEmployees();
    res.json(data);
  } catch (e) {
    res.status(500).json({ error: "db_error", message: e.message });
  }
...
app.get("/api/bookings/upcoming", requireAdminWebApp, async (req, res) => {
  try {
    const data = await dbUpcomingBookings();
    res.json(data);
  } catch (e) {
    res.status(500).json({ error: "db_error", message: e.message });
  }
...
app.get("/api/bookings/past", requireAdminWebApp, async (req, res) => {
  try {
    const data = await dbPastBookings();
    res.json(data);
  } catch (e) {
    res.status(500).json({ error: "db_error", message: e.message });
  }
...
app.get("/api/compta/summary", requireAdminWebApp, async (req, res) => {
  try {
    // Total par mois, par client, par prestation, parts employés / entreprise
    const { data, error } = await sb
      .from("bookings")
...
function webAppUrl() {
  // Render URL
  return process.env.WEBAPP_URL || "https://TON-RENDER.onrender.com";
}

function sendMainMenu(chatId) {
  if (!isAdmin(chatId)) {
    return bot.sendMessage(chatId, "⛔ App privée ShaSitter. Accès refusé.");
  }
...
bot.on("callback_query", async (q) => {
  ...
  if (q.data === "list_upcoming") {
    ...
    const txt = rows
      ...
      .map((b) => {
        ...
        return `#${b.id} • ${b.start_date}→${b.end_date} • ${c} • ${p} • ${b.total_chf} CHF`;
      })
      .join("\n");
  ...
  // ---- BOOKING: assign employee? (optional) ----
...
bot.on("message", async (msg) => {
  ...
  if (ps) {
    ...
    if (ps.step === "desc") {
      d.description = text;
      // image optionnel (tu pourras la mettre plus tard en SQL ou via /edit si tu veux)
      d.image_url = "";
  ...
  if (bs) {
    ...
    if (bs.step === "employee_percent") {
      ...
      try {
        ...
        const empPart = money2((total * d.employee_percent) / 100);
        const coPart = money2(total - empPart);
        ...
        const payload = {
          ...
          employee_part_chf: d.employee_id ? empPart : 0,
          company_part_chf: d.employee_id ? coPart : total,
          notes: "",
        };
